...
public class Main {
    public static void main(String[] args) {
        ...
        bookService.addBook(new Book(
                1, "Java OOP", "Ivan Petrov",
                29.99, LocalDate.now().minusMonths(8),
                "Книга по ООП на Java"
        ));

        bookService.addBook(new Book(
                2, "Clean Code", "Robert Martin",
                39.5, LocalDate.now().minusMonths(2),
                "Практики чистого кода"
        ));

        bookService.addBook(new Book(
                3, "Algorithms", "CLRS",
                49.0, LocalDate.now().minusMonths(10),
                "Algorithms description"
        ));

        System.out.println("\n--- Список книг по алфавиту ---");
        bookService.getBooks(BookService.SortType.TITLE)
                .forEach(System.out::println);

        System.out.println("\n=== Залежавшиеся книги (>6 месяцев) ===");
        bookService.getStaleBooks().forEach(System.out::println);

        System.out.println("\n=== Залежавшиеся книги (по дате поступления) ===");
        bookService.getStaleBooks(BookService.SortType.DATE_ADDED).forEach(System.out::println);

        System.out.println("\n=== Залежавшиеся книги (по цене) ===");
        bookService.getStaleBooks(BookService.SortType.PRICE).forEach(System.out::println);
        ...
        System.out.println("\n--- Добавляем книгу 2 на склад как AVAILABLE ---");
        Book restored = new Book(
                2, "Clean Code", "Robert Martin",
                39.5, LocalDate.now(),
                "Практики чистого кода"
        );
        restored.setStatus(BookStatus.AVAILABLE);
        bookService.addBook(restored);
        ...
        orderService.getOrderById(1).ifPresent(o -> {
            ...
            System.out.println("Описание книги: " + o.getBook().getDescription());
